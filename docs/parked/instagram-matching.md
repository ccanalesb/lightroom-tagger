...
**Visualizer dump-path config:** `GET`/`PUT /api/config/instagram-dump`, the `instagram_dump_path` `Config` field, `INSTAGRAM_DUMP_PATH` env override, and `update_config_yaml_instagram_dump_path` were removed in [#253](https://github.com/ccanalesb/lightroom-tagger/issues/253) after the importer was deleted in [#225](https://github.com/ccanalesb/lightroom-tagger/issues/225); frontend clients went in [#252](https://github.com/ccanalesb/lightroom-tagger/issues/252).
