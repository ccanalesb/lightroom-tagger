...
- `run(argv, config, commands) -> int` is the testable engine; `main()` loads config (when not injected) and calls `run(sys.argv[1:], None, COMMANDS)`.
