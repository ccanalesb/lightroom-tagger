...
def compute_phash(image_path: str, hash_size: int = 8) -> Optional[str]:
    ...
    path_to_try = image_path
    
    # If DNG and doesn't work, try JPG sidecar
    if image_path.lower().endswith('.dng'):
        jpg_path = image_path.replace('.DNG', '.JPG').replace('.dng', '.jpg')
        if Path(jpg_path).exists():
            path_to_try = jpg_path
    
    try:
        img = Image.open(path_to_try)
