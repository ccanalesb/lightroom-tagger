...
const getStackMembersMock = vi.fn()
...
vi.mock('../../../services/api', () => ({
  ImagesAPI: {
    ...
    getStackMembers: (...args: unknown[]) => getStackMembersMock(...args),
  },
  PerspectivesAPI: {
...
describe('CatalogTab', () => {
  beforeEach(() => {
    ...
    getStackMembersMock.mockResolvedValue({ items: [] })
  ...
  it('offers Show stack and fetches members when a stack representative row is present', async () => {
    ...
    const member: CatalogImage = { ...rep, id: 2, key: 'a/b/member.jpg', filename: 'member.jpg' }
    listCatalogMock.mockResolvedValue({ images: [rep], total: 1 })
    getStackMembersMock.mockResolvedValue({ items: [rep, member] })
    ...
    await waitFor(() => {
      expect(getStackMembersMock).toHaveBeenCalledWith(42)
    })
  })
})
