"""Tests for POST /api/images/search/nl-search (LLM filter mocked)."""
...
def test_nl_search_empty_query_400(nl_search_client):
    for body in ({"query": ""}, {"query": "   "}):
        r = nl_search_client.post("/api/images/search/nl-search", json=body)
...
def test_nl_search_mock_llm_success(nl_search_client, monkeypatch):
    ...
    r = nl_search_client.post("/api/images/search/nl-search", json={"query": "unposted only"})
...
def test_nl_search_not_json_400_includes_nl_filter(nl_search_client, monkeypatch):
    ...
    r = nl_search_client.post("/api/images/search/nl-search", json={"query": "x"})
    assert r.status_code == 400
...
def test_nl_search_extra_key_from_llm_400(nl_search_client, monkeypatch):
    ...
    r = nl_search_client.post("/api/images/search/nl-search", json={"query": "posted"})
    assert r.status_code == 400
    data = r.get_json()
