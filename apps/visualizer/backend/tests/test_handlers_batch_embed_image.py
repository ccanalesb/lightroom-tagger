...
def test_batch_embed_image_resolves_filepath_before_encode(
    ...
    monkeypatch.setattr(embed_mod, "encode_images", _encode)
    monkeypatch.setattr(path_diag_mod, "resolve_filepath", lambda _p: str(resolved_jpg))
    monkeypatch.setattr(
        path_diag_mod,
        "get_or_create_cached_image",
        lambda _db, _k, path: path,
...
@patch("database.add_job_log")
def test_batch_embed_image_preflight_continues_when_paths_inaccessible(
    mock_add_log, tmp_path, monkeypatch
) -> None:
    from jobs.handlers.embed import handle_batch_embed_image
    ...
    assert any("batch_embed_image preflight" in msg for msg in warnings), warnings
...
@patch("database.add_job_log")
def test_batch_embed_image_preflight_continues_when_majority_unreachable(
    mock_add_log, tmp_path, monkeypatch
) -> None:
    """Preflight warns but the job completes when most sampled paths are unreachable."""
    from jobs.handlers.embed import handle_batch_embed_image
...
@patch("database.add_job_log")
def test_batch_embed_image_preflight_no_warning_when_sample_clean(
    mock_add_log, tmp_path, monkeypatch
) -> None:
    from jobs.handlers.embed import handle_batch_embed_image
    ...
    monkeypatch.setattr(embed_mod, "_EMBED_PREFLIGHT_SAMPLE_SIZE", 3)
    monkeypatch.setattr(
        embed_mod,
        "encode_images",
        lambda paths, batch_size=8: np.ones((len(paths), 512), dtype=np.float32),
    ...
    monkeypatch.setenv("LIBRARY_DB", str(db_path))
    runner = _make_runner()
    handle_batch_embed_image(runner, "job-clean-preflight", {"image_type": "catalog"})
    ...
    assert not any("batch_embed_image preflight" in msg for msg in warnings), warnings
