...
def test_batch_score_should_complete_with_zero_units(
    ...
    with patch('lightroom_tagger.core.database.get_undescribed_catalog_images', return_value=[]):
...
def test_batch_score_checkpoint_skips_already_processed_triplets(
    ...
    mock_get_undescribed.return_value = [{'key': 'img1'}]
...
def test_batch_score_should_call_score_for_each_triplet(
    _mock_exists, mock_getenv, mock_config, mock_init_db, mock_get_undescribed, mock_score, _mock_add_log,
):
    from jobs.handlers import handle_batch_score

    mock_config.return_value = MagicMock(db_path='/tmp/library.db')
    mock_init_db.return_value = MagicMock()
    mock_get_undescribed.return_value = [{'key': 'img1'}, {'key': 'img2'}]
