/**
 * Pins `erf` / `erfc` against values generated by Python's `math` module — the
 * implementation the Flask backend's crowning test ran on.
 *
 * The point is not to prove the two are identical (they are not, see
 * `src/utils/erf.ts`), but to hold the error at a *measured* bound. If a future
 * change to the approximation makes it worse, this fails with the offending input
 * rather than surfacing as a lens that quietly stops being crowned.
 */
...
describe('erf / erfc against Python math', () => {
  ...
  it('stays within the documented relative error', () => {
    ...
    console.log(
      `erf/erfc vs Python: worst relative error erf=${worstErf.rel.toExponential(2)} ` +
