...
- **v4.0 Phase 14 complete** (2026-05-06) — Database & images API split: `database.py` → `database/` package (10 domain submodules, barrel re-export, 124 symbols preserved); `api/images.py` → `api/images/` package (5 family blueprints, D-09 URL prefixes); test suite mirrored; all 269+341 tests passing (REFACTOR-02, REFACTOR-03 verified).
- **v4.0 Phase 16 complete** (2026-05-06) — Recovery & path-failure unit tests: 6 new tests across `batch_describe`/`batch_score` (checkpoint resume, fingerprint mismatch reset, stale-all-processed completion) and orphan recovery for `batch_score`; TEST-02 confirmed satisfied by Phase 12 embed tests; full suite 347/347 passing (TEST-01, TEST-02 verified).
