...
FLAG_FILE="$FLAG_DIR/ui-validation-required"
mkdir -p "$FLAG_DIR"
...
# Treat UI changes broadly: any frontend source edit that is not a test file.
if [[ "$REL_PATH" =~ ^apps/visualizer/frontend/src/ ]] \
  && [[ "$REL_PATH" =~ \.(tsx|ts|css)$ ]] \
  && [[ ! "$REL_PATH" =~ (__tests__/|\.test\.|\.spec\.) ]]; then
  touch "$FLAG_FILE"
fi
