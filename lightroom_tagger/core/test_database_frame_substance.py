...
def test_delete_frame_substance_override_clears_override(tmp_path) -> None:
    conn = init_database(str(tmp_path / "library.db"))
    key = store_image(
        conn,
        {"date_taken": "2024-01-10", "filename": "a.jpg", "filepath": "/a.jpg"},
    ...
    conn.commit()
    assert is_frame_substance_flagged(conn, key) is False
    assert delete_frame_substance_override(conn, key) is True
    conn.commit()
    assert is_frame_substance_flagged(conn, key) is True
    assert delete_frame_substance_override(conn, key) is False
    conn.close()
