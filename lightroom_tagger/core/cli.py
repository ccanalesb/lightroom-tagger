...
def create_parser(commands) -> argparse.ArgumentParser:
    """Create argument parser with subcommands derived from *commands*."""
...
def _apply_global_args_to_config(args, config):
    if args.catalog:
...
def run(argv, config, commands) -> int:
    """Parse *argv*, apply global overrides, dispatch via *commands*, return exit code."""
    ...
    _apply_global_args_to_config(args, config)
...
def main():
    ...
    return run(sys.argv[1:], config, COMMANDS)
